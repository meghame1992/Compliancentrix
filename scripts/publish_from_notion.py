...
header a {{ color: {GOLD}; text-decoration: none; font-family: Georgia, serif;
...
def article_html(title, content_type, date_str, body_html, topics, sources_html):
    ...
    badge_color_text = "#fff" if bc != GOLD else NAVY
    summary = first_45_words(body_html)
...
<div class="hero">
  <div class="badge" style="background:{bc};color:{badge_color_text}">{badge_label}</div>
  <h1>{title}</h1>
  <div class="meta">
    <span>📅 {date_str}</span>
...
<meta name="description" content="Daily compliance intelligence from India and beyond.">
